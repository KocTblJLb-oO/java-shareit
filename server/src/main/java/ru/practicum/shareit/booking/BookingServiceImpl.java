...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDtoResponse create(BookingDto bookingDto, Long bookerId) {
        ...
        User owner = UserMapper.toUser(
                userService.findUserById(
                        itemService.findItemById(bookingDto.getItemId()).getOwner())
        );

        ItemRequest itemRequest = null;
        if (itemService.findItemById(bookingDto.getItemId()).getRequestId() != null) {
            itemRequest = ItemRequestMapper.toItemRequest(
                    itemRequestService.getItemRequestById(itemService.findItemById(bookingDto.getItemId()).getRequestId()
                    ),
                    UserMapper.toUser(userService.findUserById(itemRequestService.getItemRequestById(itemService.findItemById(bookingDto.getItemId()).getRequestId()).getRequester())
                    )
            );
        }

        Item item = ItemMapper.toItem(itemService.findItemById(bookingDto.getItemId(), owner.getId()), owner, itemRequest);
